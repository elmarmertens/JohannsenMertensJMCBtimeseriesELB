PROGRAM main
  ...
  INTEGER :: gapOrderKey = 123
  ! gapOrderKey is a 3-digit code that specifies the order of variables within the gap block:
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: DRAWf, DRAWrbarvar, DRAWmaxlambda, DRAWshockslopes
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: GELMANf, GELMANrbarvar, GELMANshockslopes
  ...
  INTEGER :: ndxRealrate, ndxPIbar, ndxRbar, ndxPIgap, ndxPInoise, ndxUgap, ndxINTgap, ndxSHADOWRATE, ndxRGAPSV 
  ...
  Nyield = 0  ! legacy variable, just needed to make getarguments work as for other model variants
  p      = 2  ! just a default, can be overriden by command line arguments

  Nsvorth = 2 ! this could actually have been defined as a parameter 
  ...
  Nstates   = 9 ! see list of ndxXXX below
 ...
 if (doZLBasZeroes .eqv. .true.) then
  ...
  filext = '.NOMASVAR.' // trim(datalabel) // '.VARlags' // trim(int2str(p)) // '.order' // trim(int2str(gapOrderKey)) // '.ar1svcor.T' // trim(int2str(T)) // '.Tjumpoff' // trim(int2str(T0)) // '.dat'
  ...
  ALLOCATE (DRAWmaxlambda(1,Nsim,Nstreams), DRAWf(Nf,Nsim,Nstreams), DRAWshockslopes(Nshockslopes,Nsim,Nstreams), DRAWrbarvar(1,Nsim,Nstreams), DRAWstates(Nstates,0:T,Nsim,Nstreams), STAT=status)
  ...
  !$OMP PARALLEL DO SHARED(DRAWy,DRAWstates,DRAWsvol,DRAWf,DRAWshockslopes,DRAWhorthvar,DRAWhorthbar,DRAWhorthrho,DRAWsvolorth, DRAWhSigma,DRAWhbar,DRAWhrho,DRAWrbarvar,DRAWmaxlambda), FIRSTPRIVATE(ZLBrejection,doPDF,doPInoise, gapOrderKey, Nstreams, Nsim,Burnin,T,y,yNaN,zlb,Nstates,Nx, Nf, Eshockslopes0, sqrtVshockslopes0, Ef0, sqrtVf0, Eh0, sqrtVh0, minSV, maxSV, hSigmaT, hSigmaDof, hrho0, hrhoV0, Ehorth0, Vhorth0, minSVorth, maxSVorth, horthvarT, horthvarDof, horthrho0, horthrhoV0, rbarvarT, rbarvarDof, NNxx), PRIVATE(VSLstream,TID,errcode,timer) SHARED(Ny,Nbar,Ngap,Nshock,Nshockslopes,p,Nsv,Nhsigma,Nsvorth,NNyy) DEFAULT(NONE) SCHEDULE(STATIC)
  ...
  DO j=1,Nstreams
     ...
     call thissampler(ZLBrejection, ELBound, doPDF, doPInoise, gapOrderKey, T, Ny, y, yNaN, zlb, NNyy, NNxx, DRAWy(:,:,:,:,j), Nforecastdraws, maxhorizons, DRAWstates(:,:,:,j), Nstates, Nx, Nshock, Nbar, Ngap, p, DRAWf(:,:,j), Nf, Ef0, sqrtVf0, DRAWmaxlambda(:,:,j), DRAWsvol(:,:,:,j), Nsv, Eh0, sqrtVh0, minSV, maxSV, DRAWhbar(:,:,j), DRAWhSigma(:,:,j), NhSigma, hSigmaT, hSigmaDof, DRAWhrho(:,:,j), hrho0, hrhoV0,  DRAWsvolorth(:,:,:,j), Nsvorth, Ehorth0, Vhorth0, minSVorth, maxSVorth, DRAWhorthbar(:,:,j), DRAWhorthvar(:,:,j), horthvarT, horthvarDof, DRAWhorthrho(:,:,j), horthrho0, horthrhoV0, DRAWrbarvar(:,:,j), rbarvarT, rbarvarDof, DRAWshockslopes(:,:,j), Nshockslopes, Eshockslopes0, sqrtVshockslopes0, Nsim, Burnin, VSLstream, timer)
  ...
  WRITE(4,'(a10)') 'PRIORS:'
  WRITE(4,'(a20,F4.2, a6, I4)') 'rbarvar = ', rbarvarT / dble(rbarvarDof - 2), ' w/dof: ', rbarvarDof
  ...
  filename  = 'SVRGAP' // filext
  FORALL (k=1:Nsim,j=1:Nstreams) theta(:,(j-1) * Nsim + k) = DRAWstates(ndxRGAPSV,1:T,k,j) 
  CALL storeEstimatesOMP(theta,T,Ndraws,filename)
  WRITE (*,*) 'STORED SVRGAP'
  ...
  ALLOCATE (GELMANstates(Nstates,T), GELMANf(Nf), GELMANshockslopes(Nshockslopes), GELMANrbarvar(1), STAT=status)
  ...
  !$OMP PARALLEL SHARED(DRAWstates,GELMANstates,DRAWf,GELMANF,GELMANshockslopes, DRAWshockslopes, DRAWsvol,GELMANsvol,DRAWsvolorth,GELMANsvolorth,DRAWrbarvar,GELMANrbarvar, DRAWhorthvar, GELMANhorthvar, DRAWhSigma, GELMANhSigma, DRAWhbar, GELMANhbar, DRAWhorthbar, GELMANhorthbar, DRAWhorthrho, GELMANhorthrho, DRAWhrho, GELMANhrho)
 ...
 !$OMP DO 
  ...
  DEALLOCATE (GELMANstates, GELMANsvol, GELMANf, GELMANshockslopes, GELMANhorthvar, GELMANhorthbar, GELMANsvolorth, GELMANhSigma, GELMANhbar, GELMANrbarvar)
  ...
  DEALLOCATE (DRAWmaxlambda, DRAWstates, DRAWsvol, DRAWf, DRAWshockslopes, DRAWhSigma, DRAWhbar, DRAWrbarvar)
...
CONTAINS
  ...
  SUBROUTINE readdata(y,filename,Ny,T,T0)
    ...
    ! NOTE: readdata works also when there are more than Ny columns in input data, those will just be ignored ...
...
SUBROUTINE thissampler(ZLBrejection, ELBound, doPDF, doPInoise, gapOrderKey, T, Ny, y, yNaN, zlb, NNyy, NNxx, DRAWy, Nforecastdraws, maxhorizons, DRAWstates, Nstates, Nx, Nw, Nbar, Ngap, p, DRAWf, Nf, Ef0, sqrtVf0, DRAWmaxlambda, DRAWsvol, Nsv, Eh0, sqrtVh0, minSV, maxSV, DRAWhbar, DRAWhSigma, NhSigma, hSigmaT, hSigmaDof, DRAWhrho, hrho0, hrhoV0, DRAWsvolorth, Nsvorth, Ehorth0, Vhorth0, minSVorth, maxSVorth, DRAWhorthbar, DRAWhorthvar, horthvarT, horthvarDof, DRAWhorthrho, horthrho0, horthrhoV0,  DRAWrbarvar, rbarvarT, rbarvarDof, DRAWshockslopes, Nshockslopes, Eshockslopes0, sqrtVshockslopes0, Nsim, Burnin,VSLstream,timer)
  ...
  INTENT(INOUT) :: DRAWstates, DRAWf, DRAWshockslopes, DRAWrbarvar, VSLstream, timer
  ...
  INTENT(INOUT) :: DRAWy
  INTENT(IN)    :: ZLBrejection, doPDF, doPInoise, T, Ny, y, yNaN, zlb, Nforecastdraws, maxhorizons, NNyy, NNxx, Nstates, Nx, Nw, Nbar, Ngap, p, Nf, Nsim, Burnin, Ef0, sqrtVf0, rbarvarT, rbarvarDof, Nshockslopes, Eshockslopes0, sqrtVshockslopes0
  ...
  INTEGER :: ndxPIbar, ndxRbar, ndxUgap, ndxINTgap, ndxPIgap, ndxPInoise, ndxGapStart, ndxGapStop, ndxGapLagStop
  INTEGER :: ndxShockPIbar, ndxShockRbar, ndxShockGapStart, ndxShockPIgap, ndxShockUgap, ndxShockINTgap, ndxShockPInoise
  INTEGER :: ndxSVpibar, ndxSVpigap, ndxSVugap, ndxSVintgap, ndxSVpinoise
  INTEGER :: yndxInflation, yndxPolicyrate, yndxUgap
  ...
  ! computation of SV in rgap
  double precision :: hrgap(Nx), rgapVol(0:T), volvec(Nw)
	...
	horthrho      = PREV_HORTHRHO(:,lastInStack) 
     ...
     DO WHILE (.NOT. OK .AND. (i < zlbShakes))
        ...
        IF (j > BURNIN) THEN
           ...
           ! real-rate gap Vol
           DO k=1,T
              ! a) setup selection vector
              hrgap = 0.0d0
              hrgap(ndxINTgap) = 1.0d0
              hrgap = hrgap - A(ndxPIgap,:,k)
              ! b) compute volvec = hrgap * Bsv
              volvec = 0.0d0
              call DGEMV('t',Nx,Nw,1.0d0,Bsv(:,:,k),Nx,hrgap,1,0.0d0,volvec,1)
              ! c) compute sqrt(volvec * volvec')
              rgapVOL(k) = sqrt(sum(volvec ** 2))
           END DO

           ! k = 200
           ! call savemat(Bsv(:,:,k), 'Bsv.debug')
           ! call savemat(A(:,:,k), 'A.debug')
           ! print *, ndxINTgap, ndxPIgap, Nx, rgapVol(k)
           ! stop 3
        END IF
     ...
     IF (OK) THEN ! move forward
        ...
        IF (j > BURNIN) THEN
           ...
           DRAWstates(4,:,thisdraw) = x(ndxPIgap,:)
           DRAWstates(5,:,thisdraw) = x(ndxUgap,:)
           DRAWstates(6,:,thisdraw) = x(ndxINTgap,:)
           DRAWstates(7,:,thisdraw) = x(ndxPInoise,:)
           DRAWstates(8,:,thisdraw) = istar
           DRAWstates(9,:,thisdraw) = rgapVol
	   ...
	   DRAWhorthrho(:,thisdraw)    = horthrho
           ...
           if (doPDF) then
              ...
              h_sim0(ndxSVpigap:ndxSVintgap)     = PREV_SVOL(:,T,lastInstack)
              ...
              hbar_sim0(ndxSVpigap:ndxSVintgap)     = PREV_hbar(:,lastInstack)
              ! c) hrho_sim0
              hrho_sim0(ndxSVpibar)                 = PREV_horthrho(1,lastInstack)
              hrho_sim0(ndxSVpinoise)               = PREV_horthrho(2,lastInstack)
              hrho_sim0(ndxSVpigap:ndxSVintgap)     = PREV_hrho(:,lastInstack)
              ! d) sqrtVhshock_sim0
              ...
              sqrtVhshock_sim0(ndxSVpigap:ndxSVintgap,ndxSVpigap:ndxSVintgap) = PREV_HSIGMA(:,:,lastInstack)
