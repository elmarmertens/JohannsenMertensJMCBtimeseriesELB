PROGRAM main
  ...
  INTEGER :: ndxRealrate, ndxPIbar, ndxRbar, ndxLONGRbar, ndxPIgap, ndxPInoise, ndxUgap, ndxINTgap, ndxLONGINTgap, ndxSHADOWRATE, ndxRGAPSV !, ndxLONGSHADOW, ndxLONGRATE, ndxZLBPROB1, ndxZLBPROB2, ndxZLBPROB3
  ...
  Nstates   = 8 + 2 * Nyield + 1 ! see list of ndxXXX below
 ...
 if (doZLBasZeroes .eqv. .true.) then
  ...
  filext = '.SPECTREVAR.' // trim(datalabel) // '.VARlags' // trim(int2str(p)) // '.order' // trim(int2str(gapOrderKey)) // '.ar1svcor.T' // trim(int2str(T)) // '.Tjumpoff' // trim(int2str(T0)) // '.dat'
  ...
  lambda1Vf = 0.5d0
  lambda2Vf = 0.2d0
  call minnesotaVCVsqrt(sqrtVf0, Ngap, p, lambda1Vf, lambda2Vf)
  ...
  !$OMP PARALLEL DO SHARED(DRAWy,DRAWavgtermpremia,DRAWstates,DRAWsvol,DRAWf,DRAWshockslopes,DRAWhorthvar,DRAWhorthbar,DRAWhorthrho,DRAWsvolorth, DRAWhSigma,DRAWhbar,DRAWhrho,DRAWrbarvar,DRAWmaxlambda), FIRSTPRIVATE(ZLBrejection,doPDF,doPInoise, gapOrderKey, Nstreams, Nsim,Burnin,T,y,yNaN,zlb,Nstates,Nx, Nf, Eshockslopes0, sqrtVshockslopes0, Ef0, sqrtVf0, Eh0, sqrtVh0, minSV, maxSV, hSigmaT, hSigmaDof, hrho0, hrhoV0, Ehorth0, Vhorth0, minSVorth, maxSVorth, horthvarT, horthvarDof, horthrho0, horthrhoV0, rbarvarT, rbarvarDof, NNxx), PRIVATE(VSLstream,TID,errcode,timer) SHARED(Nyield,Ny,Nbar,Ngap,Nshock,Nshockslopes,p,Nsv,Nhsigma,Nsvorth,NNyy) DEFAULT(NONE) SCHEDULE(STATIC)
...
SUBROUTINE thissampler(ZLBrejection, ELBound, doPDF, doPInoise, gapOrderKey, T, Ny, Nyield, y, yNaN, zlb, NNyy, NNxx, DRAWy, Nforecastdraws, maxhorizons, DRAWavgtermpremia, DRAWstates, Nstates, Nx, Nw, Nbar, Ngap, p, DRAWf, Nf, Ef0, sqrtVf0, DRAWmaxlambda, DRAWsvol, Nsv, Eh0, sqrtVh0, minSV, maxSV, DRAWhbar, DRAWhSigma, NhSigma, hSigmaT, hSigmaDof, DRAWhrho, hrho0, hrhoV0, DRAWsvolorth, Nsvorth, Ehorth0, Vhorth0, minSVorth, maxSVorth, DRAWhorthbar, DRAWhorthvar, horthvarT, horthvarDof, DRAWhorthrho, horthrho0, horthrhoV0,  DRAWrbarvar, rbarvarT, rbarvarDof, DRAWshockslopes, Nshockslopes, Eshockslopes0, sqrtVshockslopes0, Nsim, Burnin,VSLstream,timer)
  ...
  INTENT(INOUT) :: DRAWy
  INTENT(IN)    :: ZLBrejection, doPDF, doPInoise, T, Ny, Nyield, y, yNaN, zlb, Nforecastdraws, maxhorizons, NNyy, NNxx, Nstates, Nx, Nw, Nbar, Ngap, p, Nf, Nsim, Burnin, Ef0, sqrtVf0, rbarvarT, rbarvarDof, Nshockslopes, Eshockslopes0, sqrtVshockslopes0
  ...
  DO WHILE (thisdraw <= Nsim)
     ...
     IF (j == 1) THEN
        ...
        ! hSigma
        call iwishcholDraw(sqrtVhshock, hSigmaT, hSigmaDof, Nsv, VSLstream) ! return LHS upper factor
        ! b) draw hgap
	...
	horthrho      = PREV_HORTHRHO(:,lastInStack) 
     ...
     IF (OK) THEN ! move forward
        ...
        IF (j > BURNIN) THEN
           ...
           DRAWstates(6+2*Nyield+3,:,thisdraw) = rgapVol

           DRAWsvol(:,:,thisdraw)  = SVol
           DRAWhbar(:,thisdraw)    = hbar
	   DRAWhrho(:,thisdraw)    = hrho
           call vech(DRAWhsigma(:,thisdraw), sqrtVhshock) ! note: requires Upper sqrtVhshock
