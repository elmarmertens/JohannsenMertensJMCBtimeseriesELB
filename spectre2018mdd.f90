PROGRAM main
  ...
  filext = '.SPECTREVAR.' // trim(datalabel) // '.VARlags' // trim(int2str(p)) // '.order' // trim(int2str(gapOrderKey)) // '.ar1svcor.T' // trim(int2str(T)) // '.Tjumpoff' // trim(int2str(T0)) // '.run' // trim(int2str(runnum)) // '.dat'
  ...
  if (doMDD) then
     ...
     allocate (theta(Nsv + 1, 2),theta2(Nsv + 1, 2))
     ...
     call vdcdfnorm((Nsv + 1) * 2,theta,theta2)
  ...
  !$OMP PARALLEL DO SHARED(DRAWMDDlogprior,DRAWMDDloglike,DRAWy,DRAWavgtermpremia,DRAWstates,DRAWsvol,DRAWf,DRAWshockslopes,DRAWhorthvar,DRAWhorthbar,DRAWhorthrho,DRAWsvolorth, DRAWhSigma,DRAWhbar,DRAWhrho,DRAWrbarvar,DRAWmaxlambda, runnum, seed), FIRSTPRIVATE(ZLBrejection,doPDF,doMDD,doPInoise, gapOrderKey, Nstreams, Nsim, Nparticles, Burnin,T,y,yNaN,zlb,Nstates,Nx, Nf, Eshockslopes0, sqrtVshockslopes0, Ef0, sqrtVf0, Eh0, sqrtVh0, minSV, maxSV, hSigmaT, hSigmaDof, hrho0, hrhoV0, Ehorth0, Vhorth0, minSVorth, maxSVorth, horthvarT, horthvarDof, horthrho0, horthrhoV0, rbarvarT, rbarvarDof, NNxx), PRIVATE(VSLstream,TID,errcode,timer) SHARED(Nyield,Ny,Nbar,Ngap,Nshock,Nshockslopes,p,Nsv,Nhsigma,Nsvorth,NNyy) DEFAULT(NONE) SHARED(fPriorTruncationProbLog,hrhoPriorTruncationProbLog) SCHEDULE(STATIC)
  ...
  if (doMDD) then
     ...
     ! set up theta
     Ntheta = 1 + Nf + Nshockslopes + Nsv * 2 + Nhsigma + 3
     ...
     allocate (ndxf(Nf),ndxrho(Nsv+1))
     forall (k=1:Nf)  ndxf(k)   = 1 + k
     forall (k=1:Nsv) ndxrho(k) = 1 + Nf + Nshockslopes + k
     ndxrho(Nsv+1) =  1 + Nf + Nshockslopes + Nsv * 2 + Nhsigma + 1
     call thisMDDthetaTruncationProb(gewekeTruncLogProb, Ny, p, ndxf, Nsv + 1, ndxrho, gewekeTruncationNsim, Ndraws, Ntheta, theta, VSLdefaultstream)
...
SUBROUTINE thissampler(ZLBrejection, ELBound, doPDF, doMDD, doPInoise, gapOrderKey, T, Ny, Nyield, y, yNaN, zlb, DRAWMDDlogprior, DRAWMDDloglike, NNyy, NNxx, DRAWy, Nforecastdraws, maxhorizons, DRAWavgtermpremia, DRAWstates, Nstates, Nx, Nw, Nbar, Ngap, p, DRAWf, Nf, Ef0, sqrtVf0, fPriorTruncationProbLog, DRAWmaxlambda, DRAWsvol, Nsv, Eh0, sqrtVh0, minSV, maxSV, DRAWhbar, DRAWhSigma, NhSigma, hSigmaT, hSigmaDof, DRAWhrho, hrho0, hrhoV0, hrhoPriorTruncationProbLog, DRAWsvolorth, Nsvorth, Ehorth0, Vhorth0, minSVorth, maxSVorth, DRAWhorthbar, DRAWhorthvar, horthvarT, horthvarDof, DRAWhorthrho, horthrho0, horthrhoV0,  DRAWrbarvar, rbarvarT, rbarvarDof, DRAWshockslopes, Nshockslopes, Eshockslopes0, sqrtVshockslopes0, Nsim, Burnin, Nparticles, VSLstream,timer)
  ...
  INTENT(INOUT) :: DRAWy
  INTENT(IN)    :: ZLBrejection, doPDF, doMDD, doPInoise, T, Ny, Nyield, y, yNaN, zlb, Nforecastdraws, maxhorizons, NNyy, NNxx, Nstates, Nx, Nw, Nbar, Ngap, p, Nf, Nsim, Burnin, Ef0, sqrtVf0, rbarvarT, rbarvarDof, Nshockslopes, Eshockslopes0, sqrtVshockslopes0
  ...
  ! MDD
  double precision, dimension(1) :: rbarvarlogGammaDof
  ...
  ! sqrtVx0 should be lower triangular left factor
  ...
  DO WHILE (thisdraw <= Nsim)
     ...
     ! if (j > burnin) WRITE (*,*) 'STEP ', j, '(TID ', tidomp(), ')'
     ...
     IF (OK) THEN ! move forward
	   ...
	   DRAWhorthrho(:,thisdraw)    = horthrho
           ...
           if (doMDD) then
              ...
              ! note: this structure mirrors quite a bit what has been done above with the _sim0 Variables; however, the latter include also PInoise stuff which is not supported by particlelike
              ABCSVhrho = 0.0d0
              ABCSVhrho(1)         = PREV_horthrho(1,lastInstack)
              ABCSVhrho(1+1:1+Nsv) = PREV_hrho(:,lastInstack)

              ABCSVhbar = 0.0d0
              ABCSVhbar(1)         = PREV_horthbar(1,lastInstack)
              ABCSVhbar(1+1:1+Nsv) = PREV_hbar(:,lastInstack)

              ABCSVsqrtVh = 0.0d0
              ABCSVsqrtVh(1,1) = sqrt(PREV_horthvar(1,lastInstack))
              ABCSVsqrtVh(1+1:1+Nsv,1+1:1+Nsv) = PREV_hSigma(:,:,lastInStack)
              ...
              if (doPInoise) then
                 ...
                 ! DRAWMDDloglike(thisdraw) = 0.0d0
                 call particlelike(DRAWMDDloglike(thisdraw), ELBound, Nyield, T, Ny, y, yNaN, atELB, Nparticles, Nx-1, Nw-1, Nsv+Nsvorth-1, p, PREV_f(:,lastInStack), Nf, PREV_shockslopes(:,lastInStack), Nshockslopes, ABCSVhbar, ABCSVhrho, ABCSVsqrtVh, sqrt(PREV_rbarvar(lastInStack)), VSLstream)
              end if ! doPInoise
...
SUBROUTINE particlelike(loglike, elbound, Nyield, T, Ny, y, yNaN, atELB, Nparticles, Nx, Nw, Nsv, p, f, Nf, gapshockslopes, Ngapshockslopes, hbar, hrho, sqrtVh, rbarvol, VSLstream)
