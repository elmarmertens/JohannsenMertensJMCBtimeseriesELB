function progressbar4xterm(fractiondone, preface, timeonly)
% PRGORSSBAR4XTERM -- adapts codes for of PROGRESSBAR to print string
% output to command line
%
% USAGE: progressbar4xterm(fractiondone, preface, timeonly)
% - fractiondone is a number betwen 0 and 1
% - timeonly is optional. If "true" only the time estimate will be printed
%   (the remaining percentage will thus be omitted from the printout).
% - preface is an optional text to be printed before the progress report
...
persistent starttime lastupdate
...
if etime(clock,lastupdate) < 30 % wait thirty seconds
   return
end

% Update progress figure title bar
if fractiondone > 0.01
    runtime     = etime(clock,starttime);
    timeleft    = runtime/fractiondone - runtime;
    timeleftstr = sec2timestr(timeleft);
    if timeonly
       titlebarstr = sprintf('%s remaining',timeleftstr);
    else
